...
本模块是 **AI 编排层**：负责调用 Rizin HTTP 服务获取静态分析/反编译结果，并将结果组织成上下文，调用 LLM 生成“函数级分析”与“最终恶意软件报告”。
...
**本模块做什么**
- 定义并实现“分析流水线”（上传样本 -> Rizin 深度分析 -> 拉取元数据/函数/字符串/调用图 -> 批量反编译 -> LLM 批量函数分析 -> 过滤关键函数 -> LLM 总结报告）。
- 封装 Rizin HTTP 服务的客户端与接口契约（endpoint key -> path 映射、超时、重试）。
- 封装两类 LLM Agent，并强制 **JSON object** 输出格式，供后端落库。

**本模块不做什么**
- 不负责持久化、任务排队、去重（这些在 `backend/`）。
- 不负责前端展示与交互（这些在 `frontend/`）。
- 不负责多样本并发隔离（Rizin 服务侧存在全局状态，单并发策略在 `backend/worker` 强制）。
...
- `rizin_client.py`
  - `RizinClient`: Rizin HTTP 客户端；封装请求、超时、重试、返回解析。
- `agent_core.py`
...
- `config.yaml` / `config.yaml.example`
  - Rizin 插件 base_url 与 endpoints 映射；两类 Agent 的模型、key、prompt 路径、并发等。
- `exceptions.py`
  - `RizinBackendError`, `LLMResponseError` 等异常类型。
- `main.py`
...
1. **健康检查**：`RizinClient.check_health()`
2. **上传样本**：`RizinClient.upload_file(filename, content, content_type)`
3. **深度分析**：`RizinClient.trigger_analysis()`（默认 `aaa`）
4. **元数据**：`RizinClient.get_metadata()`（来自 Rizin `ij`）
5. **函数列表**：`RizinClient.get_functions()`（来自 Rizin `aflj`）并整理为 `functions_data`
6. **字符串**：`RizinClient.get_strings()`（来自 Rizin `izj`）
7. **全局调用图**：`RizinClient.get_callgraph()`（来自 Rizin `agC json`）
8. **批量反编译**：`RizinClient.get_decompiled_codes_batch(func_names)`（POST `/decompile_batch`）
...
- `fcn.*` 自动命名函数（Rizin 常见前缀）
- 常见入口点：`main/WinMain/DllMain` 及若干 CRT 启动符号（见 `_is_ai_target_function()`）

这样做的目的：减少无关函数带来的 token 成本与噪音。

---

## 4. RizinClient 与 HTTP 契约

### 4.1 endpoint key -> path 映射
`RizinClient` 使用 `config.plugins["rizin"].endpoints` 将 key 映射到 path。
- 文档/实现约定：新增/修改 Rizin HTTP 路由时，**必须同步更新** `agents/config.yaml` 的 `plugins.rizin.endpoints`。

默认 endpoints 参考：`agents/config.yaml.example`。

### 4.2 超时与重试
`RizinClient` 基于 httpx：
- 默认 timeout：`httpx.Timeout(60.0, connect=10.0)`，单个接口可覆盖。
- transport 重试：`httpx.AsyncHTTPTransport(retries=3)`。
- 分析与反编译超时更长：
  - `trigger_analysis()`：120s
  - `get_decompiled_codes_batch()`：600s

### 4.3 重要返回形状（简要）
- `POST /decompile_batch` 入参：JSON 数组 `List[str]`（地址或函数名）
- `POST /decompile_batch` 出参：`[{"address": "<name_or_addr>", "code": "..."}, ...]`
  - 注意：rz_pipe 端会“吞异常并跳过失败项”，上游必须容忍缺项。
...
- `plugins.rizin.base_url`：Rizin HTTP 服务地址
- `plugins.rizin.endpoints`：key->path
...
- `PTW_RIZIN_BASE_URL`：覆盖 `plugins.rizin.base_url`（Docker/生产常用）。

---

## 7. 并发与全局状态约束（重要）

- Rizin 服务（`module/rz_pipe`）维护“当前打开二进制”的 **全局 analyzer** 状态。
...
### 8.1 新增/修改 Rizin 能力
1. 在 `module/rz_pipe/analyzer.py` 增加 `cmdj()`/`cmd()` 封装
2. 在 `module/rz_pipe/main.py` 增加路由
3. 同步 `agents/config.yaml` 的 endpoints
4. 在 `agents/rizin_client.py` 增加对应方法
...
- Rizin HTTP：默认 `http://localhost:8000`（由 `PTW_RIZIN_BASE_URL` 覆盖）
