...
- `api/endpoints.py`
  ...
  - `/api/tasks/{task_id}`：查询任务状态与结果（默认轻量；可选重字段）。
  - `/api/result/{sha256}`：按 hash 查询最近一次分析（默认轻量；可选重字段）。
  - `/api/history`：最近任务摘要。
- `models/task.py`
...
查询参数：
- `include_heavy`（可选，默认 `false`）

返回包含：
- 元信息：`task_id/status/sha256/filename/created_at/finished_at/error`
- 轻量字段（默认）：`metadata/malware_report`
- 重字段（仅 `include_heavy=true`）：`functions/strings/decompiled_code/function_xrefs/function_analyses`
...
返回说明：
- 默认轻量：返回 `metadata/malware_report` 等核心字段，不返回大体积重字段。
- 当 `include_heavy=true` 时，额外返回 `functions/strings/decompiled_code/function_xrefs/function_analyses`。
- 兼容历史语义：该接口仍不返回 `created_at/finished_at`。
